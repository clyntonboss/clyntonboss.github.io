...
function setStaggerReverse(container) {
  const items = [...container.querySelectorAll("[data-animate]")];
  const max = items.length - 1;

  items.forEach((el, idx) => {
    el.style.setProperty("--i", max - idx);
  });
}

function goToPortfolio() {
  intro.classList.remove("enter-up");
  intro.classList.add("exit-down");

  setTimeout(() => {
    intro.classList.remove("exit-down");
    intro.style.display = "none";

    sideMenu.style.display = "block";
    sideMenu.classList.add("enter-up");
  }, 400);
}
