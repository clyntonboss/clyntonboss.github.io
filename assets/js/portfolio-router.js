document.addEventListener("DOMContentLoaded", () => {
  ...
  let isInsideFormacoesCategory = false;
...
function animateTransition(updateCallback) {
  ...
  menuLinks.forEach(link => {
    link.addEventListener("click", e => {
      ...
      isInsideFormacoesCategory = false; // 🔹 reset ao mudar de seção
      ...
      console.log("SECTION KEY:", key, "INSIDE CATEGORY:", isInsideFormacoesCategory);
      
      const section = sections[key];
      if (!section) return;
      
      // 🟡 CASO ESPECIAL
      if (key === "formacoesComplementares" && isInsideFormacoesCategory) {
        ...
        // 🔹 reseta estado
        isInsideFormacoesCategory = false;
...
document.addEventListener("click", e => {
  ...
  isInsideFormacoesCategory = true;
