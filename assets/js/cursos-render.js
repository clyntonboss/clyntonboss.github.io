...
function trocarModoInterno(mode) {
  ...
  const wrapper = alvo.querySelector('.view-height-wrapper');
  if (wrapper) {
    // 1️⃣ captura altura atual
    const alturaAtual = wrapper.offsetHeight;
  
    // 2️⃣ força altura fixa antes da transição
    wrapper.style.height = alturaAtual + 'px';
    wrapper.offsetHeight; // força reflow
  
    // 3️⃣ mede altura nova
    const alturaNova = wrapper.scrollHeight;
  
    // 4️⃣ aplica animação
    wrapper.style.height = alturaNova + 'px';
  
    // 5️⃣ após animação, remove altura fixa
    wrapper.addEventListener('transitionend', function limparAltura(e) {
      if (e.propertyName === 'height') {
        wrapper.style.height = 'auto';
        wrapper.removeEventListener('transitionend', limparAltura);
      }
    });
  }

  // 🔹 animação de altura e fade
  requestAnimationFrame(() => {
    ativarTransicao(alvo);
  
    // 🔹 scroll suave para topo da seção, depois da animação
    if (mode !== "block") {
...
function renderBlocoCurso(index) {
  ...
  const wrapper = container?.querySelector(".view-height-wrapper"); // 🔹 NOVO: wrapper para animar altura
  ...
  requestAnimationFrame(() => {
    ...
    // 🔹 1️⃣ mede e trava altura ANTES de mexer em classes (AGORA no wrapper)
    const alturaAtual = wrapper.offsetHeight;
    wrapper.style.height = alturaAtual + "px";
    ...
    setTimeout(() => {
      ...
      // 🔹 4️⃣ espera 1 frame e mede nova altura (wrapper)
      requestAnimationFrame(() => {
        const alturaNova = wrapper.scrollHeight;

        if (Math.abs(alturaNova - alturaAtual) > 1) {
          wrapper.style.height = alturaNova + "px";
        } else {
          wrapper.style.height = "auto";
        }

        // mantém fade no conteúdo
        content.classList.add("is-visible");
      });

      // 🔹 Remove altura fixa após transição
      wrapper.addEventListener("transitionend", function limparAltura(e) {
        if (e.propertyName === "height") {
          wrapper.style.height = "auto";
...
function renderFluxoCursos(cursos) {
  ...
  // 🔹 Adiciona wrapper para animar altura
  let wrapper = container.querySelector(".view-height-wrapper");
  if (!wrapper) {
    wrapper = document.createElement("div");
    wrapper.className = "view-height-wrapper";
    // move todo conteúdo dentro do wrapper
  ...
  // 🔹 Aplica animação de altura para o wrapper
  requestAnimationFrame(() => {
    const alturaNova = wrapper.scrollHeight;
    if (Math.abs(alturaNova - alturaAtual) > 1) {
      wrapper.style.height = alturaNova + "px";
...
function renderListaCursos(cursos) {
  ...
  // 🔹 Cria ou seleciona wrapper para animação
  let wrapper = container.querySelector(".view-height-wrapper");
  if (!wrapper) {
    wrapper = document.createElement("div");
    wrapper.className = "view-height-wrapper";
