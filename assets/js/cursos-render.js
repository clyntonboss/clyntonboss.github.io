...
const datasetCategoriaView = [...datasetCategoria].sort((a, b) => {
  const diffData =
...
function trocarModoInterno(mode) {
  ...
  if (mode === "flow") renderFluxoCursos(datasetCategoriaView);
  if (mode === "list") renderListaCursos(datasetCategoriaView);
  if (mode === "grid") renderGradeCursos(datasetCategoriaView);
...
function renderBlocoCurso(index) {
  ...
  const curso = datasetCategoriaView[index];
  if (!curso) return;
  ...
  setTimeout(() => {
    ...
    if (indicator) {
      indicator.textContent = `${index + 1} / ${datasetCategoriaView.length}`;
    }
    ...
    if (index === datasetCategoriaView.length - 1) {
...
document.addEventListener("click", (event) => {
  ...
  if (
    btn.id === "next-course" &&
    cursoAtualIndex < datasetCategoriaView.length - 1
  ) {
    renderBlocoCurso(cursoAtualIndex + 1);
  }
